...
firstHalf str
 | odd (length str) = take (halfLength + 1) str
 | otherwise = take halfLength str
...
capitalize str = unwords [[toUpper first] ++ last | (first:last) <- (words str)]
...
powers k max
  | k < 2 = []
  | otherwise = takeWhile (\x -> x <= max) [k^i | i <- [0 .. max]]
...
joinToLength len list = [i ++ j | i <- list,  j <- list, (length (i ++ j)) == len]
...
-- sumRights ((Left a):rest) = sumRights rest
-- sumRights ((Right b):rest) = b +  sumRights rest
-- sumRights [] = 0

-- challenge way
sumRights :: [Either a Int] -> Int
sumRights list = sum (map (either (\l -> 0) (\r -> r)) list)
-- answer - here because its cool
-- sumRights = sum . map (either (const 0) id)
--      id just returns value (identity)
...
multiCompose (f:rest) val = (f . (multiCompose rest)) val
...
-- $ reverses order of application, meaning $x will be applied to all gs, but after
-- each application will be (g x)
...
interpreter list = interpreter' list 0 0
interpreter' [] x y = []
interpreter' (c:cs) x y
  ...
  | c == "printX" = (show x) : interpreter' cs x y
  | c == "printY" = (show y) : interpreter' cs x y
  | otherwise  = []
