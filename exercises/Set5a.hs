...
get :: UpDown -> Int
get (IncreasingMode counter) = counter
get (DecreasingMode counter) = counter

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (IncreasingMode counter) = IncreasingMode (counter + 1)
tick (DecreasingMode counter) = DecreasingMode (counter - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (DecreasingMode counter) = IncreasingMode counter
toggle (IncreasingMode counter) = DecreasingMode counter
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix col1 col2) = zipWith (\x y -> (x + y) / 2.0) (rgb col1) (rgb col2) 
rgb (Invert col) = map (\x -> 1.0 - x) (rgb col)
...
fromBin' :: Bin -> Int -> Int
fromBin' End counter = 0
fromBin' (O bin) counter = fromBin' bin (counter + 1)
fromBin' (I bin) counter = 2^counter + (fromBin' bin (counter + 1))

toBin :: Int -> Bin
toBin val
  | val <= 0 = O End
  | otherwise = inc (toBin (val - 1))
