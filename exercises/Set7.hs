...
member elem (Set (x:xs))
  | x == elem = True
  | otherwise = member elem (Set xs)

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add elem (Set (x:xs))
  | member elem (Set (x:xs)) = Set (x:xs)
  | otherwise = Set (sort (elem:x:xs))
add elem emptySet = Set([elem])
...
data State = Start | Eggs | Flour | Sugar | FlourSugar | Mixed | Error | Finished
  deriving (Eq,Show)
...
average (val :| list) = (foldr (+) val list) / (fromIntegral (length list + 1))

-- ------------------------------------------------------------------------------
-- -- Ex 5: reverse a NonEmpty list.

-- Examples:
--   reverseNonEmpty (1.0 :| [])  ==>  1.0
reverseNonEmpty :: NonEmpty a -> NonEmpty a
reverseNonEmpty (val :| list) = swap (val :| (reverse list))

swap :: NonEmpty a -> NonEmpty a
swap (val :| (x:xs)) = (x :| (xs ++ [val]))
swap nonEmpty = nonEmpty
...
passwordAllowed str (ContainsSome (s:tr)) = case elem s str of 
  True -> True
  False -> passwordAllowed str (ContainsSome tr)
passwordAllowed str (DoesNotContain []) = True
passwordAllowed str (DoesNotContain (s:tr)) = case elem s str of 
  True -> False
  False -> passwordAllowed str (DoesNotContain tr)
...
operation op (Literal valA) (Literal valB) = Operation op (Literal valA) (Literal valB)
operation op arA arB = Operation op arA arB

evaluate :: Arithmetic -> Integer
evaluate (Literal val) = val
evaluate (Operation "+" (Literal valA) (Literal valB)) = valA + valB 
evaluate (Operation "*" (Literal valA) (Literal valB)) = valA * valB 
evaluate (Operation op arA arB) = evaluate (Operation op (Literal (evaluate arA)) (Literal (evaluate arB)))

render :: Arithmetic -> String
render (Literal val) = show val
render (Operation op (Literal valA) (Literal valB)) = "(" ++ show valA ++ op ++ show valB ++ ")"
render (Operation op valA valB) = "(" ++ (render valA) ++ op ++ (render valB) ++ ")"
